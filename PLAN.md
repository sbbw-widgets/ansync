...
- [ ] **Step 7** — `input` uinput — Android como kbd/touch/stylus para PC + reverse para controlar Android vía AccessibilityService
  ...
  - [ ] **7d** — Companion: MediaProjection capture → MediaCodec H.264 → QUIC client via Rust NDK + JNI a `quinn` (mismo wire format que el daemon, cero compat overhead).
    ...
    - [ ] **7d-2** — Real QUIC dial: `quinn::Endpoint::connect` + cert Ed25519 + ALPN `ansync/1` + pinning contra pubkey del daemon. `StreamKind::Video` push + `StreamKind::Input` recv.
    - [ ] **7d-3** — Kotlin: `MediaProjection` capture surface + `MediaCodec` H.264 encoder loop → `nativeSendVideoChunk`.
