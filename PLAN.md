...
- [ ] **Step 9.5** — Integration glue (pre-Step 10) — sin esto no se prueba end-to-end
  ...
  - [ ] **9.5c** — Companion `streams_accept_loop` maneja `StreamKind::Input` inbound del daemon → push a `AccessibilityService` queue (ya wired el dispatch en 7e).
  - [ ] **9.5d** — Daemon abre `StreamKind::Input` outbound en `ShowScreen` para empujar host→device input. Mirror window capture key/mouse → encodea `InputMessage` → write_frame.
